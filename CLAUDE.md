...
# 指定自定义工具目录（默认 ~/.ai_agent/tools）
uv run python -m ai_agent.main --tools-dir ~/my-tools

# 指定技能目录
uv run python -m ai_agent.main --skills-dir ~/my-skills
...
循环开始前，可选的 **规划器** (`core/planner.py`) 通过启发式算法评估任务复杂度（1-10），将复杂任务分解为有依赖关系的步骤。规划器支持技能模板匹配（`plan_with_skill()`），LLM 规划时自动注入技能元数据作为参考。

### 模块结构

```
ai_agent/
├── __init__.py          # 公共 API 导出（Agent, AgentConfig, Skill, SkillRegistry 等）
├── config.py            # AgentConfig 数据类（所有配置项含 skills/tools_dir）
├── main.py              # CLI 入口（argparse + 交互式 Shell，/tools /skills /state /memory）
├── example.py           # 使用示例
├── core/
│   ├── agent.py         # Agent 主类：ReAct 循环、上下文裁剪、并发执行、错误恢复、会话持久化
│   │                    #   _validate_tool_messages() 预检防止 tool 消息 API 400 错误
│   │                    #   _register_use_skill_tool() 注册 use_skill 工具
│   ├── memory.py        # 短期记忆（滑动窗口）、工作记忆（任务状态）、长期记忆（SQLite）
│   ├── planner.py       # 复杂度评估、Plan/PlanStep、LLM 规划与重新规划、技能集成
│   └── skills.py        # 技能系统：Skill/SkillRegistry/SkillStep、Skill.md 加载、
│                        #   渐进式披露、YAML frontmatter 解析
├── llm/
...
│   │                    #   load_tools_from_directory() 动态加载 @tool 装饰的 .py 文件
...
~/.ai_agent/
├── tools/               # 自动加载的自定义工具（如 weather.py）
├── skills/              # 技能目录（Skill.md 文件）
├── sessions/            # 会话状态存储（_auto_{hash}_{uuid}.json）
...
- **动态工具加载**：`registry.py:load_tools_from_directory()` 使用 `importlib` 动态加载目录中所有 `@tool` 装饰的 .py 文件。Agent 启动时自动从 `config.tools_dir`（默认 `~/.ai_agent/tools`）加载。`--tools-dir` CLI / `AGENT_TOOLS_DIR` 环境变量可覆盖
- **技能系统**：遵循 [Claude Code Skills 规范](https://support.claude.com/en/articles/12512198-how-to-create-custom-skills)。每个技能是一个目录，包含 `Skill.md`（YAML frontmatter + Markdown 正文）。渐进式披露：元数据注入系统提示词 → LLM 调用 `use_skill` 工具加载完整内容
- **消息验证**：`_validate_tool_messages()` 三遍校验保障 API 兼容性——向后扫描找 `assistant(tool_calls)`、匹配 `tool_call_id`、清理不完整的 `tool_calls` 数组。支持并行工具调用场景
...
- **三层记忆**：短期记忆（滑动窗口消息队列，支持 OpenAI 兼容的 tool_calls/reasoning_content 字段）、工作记忆（内存键值存储 + 步骤结果）、长期记忆（SQLite，关键词搜索 + 重要度排序 + 语义搜索）
- **上下文管理**：CJK 感知的 Token 估算（不依赖 tiktoken）。`_trim_messages()` 先截断工具结果，再将消息划分为原子组按组丢弃。原子组策略：`assistant(tool_calls) + tool*` 为一组，确保 API 兼容
...
- **任务规划**：`estimate_complexity()` 使用关键词启发式（多任务连接词、复杂操作动词、长度）。复杂度 >= `plan_threshold_complexity` 时 `Planner.create_plan()` 请求 LLM 分解任务（提示词中注入技能元数据），LLM 失败时降级为简单文本拆分。`plan_with_skill()` 按名称显式匹配技能模板
